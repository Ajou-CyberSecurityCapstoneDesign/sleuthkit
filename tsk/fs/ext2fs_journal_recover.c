...
TSK_FS_META *ext4_jrecover(TSK_FS_INFO *fs, TSK_FS_META * fs_meta, TSK_INUM_T back_inum){
    ...
    /*
    if (ext2fs_dinode_load(ext2fs, back_inum, recover_meta)) {
        free(recover_meta);
        exit(1);
    }
    */
   ...
   int i;
    ...
    if (tsk_getu32(fs->endian, recover_meta->i_flags) & EXT2_IN_EXTENTS) {
        uint32_t *addr_ptr;
        ...
        for (i = 0; i < EXT2FS_NDADDR + EXT2FS_NIADDR; i++) {
            addr_ptr[i] = tsk_gets32(fs->endian, recover_meta->i_block[i]);;
        }
    }
    */

    if(recover_meta==NULL)
        return NULL;
